...
export const canNavigate = (to: RouteLocationNormalized) => {
  try {
    ...
    // If some routes have permissions, check if any parent route allows access
    ...
    // If CASL is not properly initialized, allow access
    console.warn('CASL not properly initialized, allowing access:', error)
    return true
  }
}
